...
exports.updatePaymentStatus = async (req, res) => {
  try {
    const { TransactionId, CustomMessage, PaymentStatus, OrderId, Amount, OrderReceiptLink } = req.body;
    const pCustomeMessage = JSON.parse(CustomMessage);

    if (PaymentStatus === "Success") {
      if (pCustomeMessage.type === "Class") {
        try {
          const response = await axios.post(`${process.env.BACKEND_URL}/api/clients/class`,
            {
              classId: pCustomeMessage.classId,
            },
            {
              headers: {
                "Authorization": `Bearer ${pCustomeMessage.token}`
              }
            }
          );
      ...
      } else if (pCustomeMessage.type === "Membership") {
        try {
          const response = await axios.post(`${process.env.BACKEND_URL}/api/clients/membership`,
            {
              name: pCustomeMessage.title,
              price: pCustomeMessage.rate,
              duraion: 30,
            },
            {
              headers: {
      ...
      } else if (pCustomeMessage.type === "Cart") {
        try {
          const response = await axios.post(`${process.env.BACKEND_URL}/api/clients/orders`,
            {
            },
            {
              headers: {
